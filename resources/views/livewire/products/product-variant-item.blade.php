<div x-on:close-variant="edit = false" x-data="{edit: false}" wire:sortable.item="{{ $productVariant->id }}"
    ...
    <div x-show="edit" class="mt-4 border-t border-color-6c757d">
        <div class="grid grid-cols-4 my-4">
            <div class="mr-4">
                <x-input wire:model="productVariantForm.sku" type="text" name="sku"
                         label="{{ __('products.edit.section.options.sku.label') }}"></x-input>
            </div>
            <div class="mr-4">
                <x-input wire:model="productVariantForm.barcode" type="text" name="barcode"
                         label="{{ __('products.edit.section.options.barcode.label') }}"></x-input>
            </div>
            <div class="mr-4 w-[120px]">
                <x-input wire:model="productVariantForm.quantity" type="number" min="0" step="1" name="quantity"
                         label="{{ __('products.edit.section.options.quantity.label') }}"></x-input>
            </div>
        </div>
