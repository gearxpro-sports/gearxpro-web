    ...
    <form wire:submit="update" class="bg-white p-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div class="flex flex-col space-y-4 col-span-2">
                <x-input type="text" wire:model="categoryForm.name" name="name"
                         label="{{ __('categories.edit.name.label') }}" required></x-input>
                <x-textarea rows="10" wire:model="categoryForm.description" name="description"
                            label="{{ __('categories.edit.description.label') }}"></x-textarea>
                <div class="flex items-center space-x-4">
                    <x-primary-button>
                        {{ __('common.update') }}
                    </x-primary-button>
                    <x-danger-button type="button" wire:click="deleteParent({{ $category->id }})"
