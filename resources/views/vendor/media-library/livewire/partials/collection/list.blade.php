<ul data-media-library-component-name="{{ $this->name }}" wire:key="ul-{{ $this->name }}"
    class="{{ $sortable ? 'media-library-dragula-container' : '' }} {{ count($sortedMedia) == 0 ? 'media-library-hidden' : 'media-library-items' }} !grid !grid-cols-2 md:!grid-cols-3 xl:!grid-cols-4 !mb-4">
