...
<div wire:key='{{$product->id}}' class="grow rounded-b-md overflow-hidden min-w-[594px] h-[727px]">
...
{{--        <img src="{{ Vite::asset('resources/images/'. $image)}}" alt="">--}}
    ...
    <div class="bg-white h-[108px] flex border border-white border-x border-x-white hover:border-y-white rounded-b-md transition-all duration-300 group">
        <div class="h-full grow flex flex-col px-[20px] py-[16px] relative">
            <h3 class="z-10 text-[15px] font-semibold text-color-18181a leading-[19px]">{{ $product->name }}</h3>
            <p class="z-10 text-[12px] font-medium text-color-6c757d">{{ $product->main_desc }}</p>
...
{{--        <a href="{{route('shop.show', ['product' => $product->slug, 'country_code' => session('country_code')])}}" class="h-full w-[88px] flex items-center justify-center border-l rounded-br-md border-white group-hover:border-white group-hover:bg-color-edebe5 transition-all duration-500">--}}
{{--            <x-icons name="detail" />--}}
{{--        </a>--}}
    </div>
</div>
