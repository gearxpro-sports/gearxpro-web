...
<div class="max-w-[294px] xl:min-w-[594px] xl:max-w-[594px] xl:h-[727px] rounded-b-md overflow-hidden">
    ...
    <div class="bg-color-f2f0eb h-[108px] w-full flex border-y border-color-18181a border-x border-x-color-f2f0eb hover:border-y-color-f2f0eb rounded-b-md transition-all duration-300 group">
        <div class="h-full grow flex flex-col pl-3 xl:pl-5 py-4 relative">
            ...
            <p class="z-10 pr-4 text-[12px] font-medium text-color-6c757d group-hover:text-color-b6b9bb line-clamp-1">{{ $description }}</p>
            @if ($colors)
        ...
        <a href="{{route('shop.show', ['product' => $slug, 'country_code' => session('country_code')])}}" class="h-full min-w-[64px] xl:min-w-[88px] py-4 flex items-center justify-center xl:border-l rounded-br-md border-color-18181a group-hover:border-color-f2f0eb group-hover:bg-color-18181a transition-all duration-500">
