...
<div class="max-w-[294px] xl:min-w-[594px] xl:max-w-[594px] xl:h-[727px] rounded-b-md overflow-hidden">
    ...
    <div class="bg-color-f2f0eb w-full flex items-stretch border-y border-color-18181a border-x border-x-color-f2f0eb hover:border-y-color-f2f0eb rounded-b-md transition-all duration-300 group">
        <div class="h-full grow flex flex-col pl-3 xl:pl-5 py-4 relative overflow-hidden">
            ...
            @if ($colors)
{{--                @dump($colors)--}}
                <div class="relative flex items-center space-x-1 my-1 z-10">
                    @foreach($colors as $color)
                        ...
                        @else
                            <div class="inline-flex">
                                <span class="text-xs font-medium leading-none">+{{ (count($colors) - $loop->index) }}</span>
                            </div>
                            @break
                        @endif
                    @endforeach
        ...
        <a href="{{route('shop.show', ['product' => $product->slug, 'country_code' => session('country_code')])}}" class="min-w-[64px] xl:min-w-[88px] py-4 flex items-center justify-center xl:border-l rounded-br-md border-color-18181a group-hover:border-color-f2f0eb group-hover:bg-color-18181a transition-all duration-500">
