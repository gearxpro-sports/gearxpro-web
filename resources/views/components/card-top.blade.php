...
<div class="max-w-[294px] min-w-[268px] xl:min-w-[514px] h-[455px] sm:h-[393px] xl:h-[747px] rounded-b-md overflow-hidden border-b border-color-18181a group relative shadow">
    ...
    <a href="{{ route('shop.index', ['country_code' => session('country_code'), 'categoryId' => $id_cat])}}"
        ...
        <div class="grow flex flex-col justify-center px-[20px] relative">
            <h3 class="z-10 text-[15px] font-semibold leading-[19px]">{{ $title }}</h3>
{{--            <p class="z-10 text-[12px] font-medium text-[#6C757D] group-hover:opacity-0 transition-all duration-500 ">{!! html_entity_decode($description) !!}</p>--}}
