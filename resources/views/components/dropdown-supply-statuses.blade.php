@props(['current_status' => 'new', 'statuses' => App\Models\Supply::STATUSES])

@if (in_array($current_status, ['delivered', 'canceled']))
    <x-badge class="justify-center" color="{{ $statuses[$current_status] }}">{{ __('supply.statuses.'.$current_status) }}</x-badge>
@else
    <x-dropdown align="left" width="48">
        <x-slot name="trigger">
            <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md focus:outline-none transition ease-in-out duration-150">
                <x-badge class="py-1.5 w-full text-center" color="{{ $statuses[$current_status] }}">
                    {{ __('supply.statuses.'.$current_status) }}
        ...
        @foreach ($statuses as $status => $color)
            @if ($current_status !== $status)
                ...
                >
                <x-badge class="w-full justify-center" color="{{ $color }}">{{ __('supply.statuses.'.$status) }}</x-badge>
                </button>
            @endif
        @endforeach
        </x-slot>
